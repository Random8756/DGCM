...
class EpisodeBatch:
    ...
    def _setup_data(self, scheme, groups, batch_size, max_seq_length, preprocess):
        ...
        for field_key, field_info in scheme.items():
            ...
            elif field_key == 'adj_matrix':
                self.data.transition_data[field_key] = th.eye(groups[group],dtype=dtype,device=self.device).unsqueeze(0).repeat(max_seq_length,1,1).unsqueeze(0).repeat(batch_size,1,1,1)
            else:
